...
<div class='panel panel-default'>
    ...
    <form action="{{ CRUDBooster::mainpath('add-save') }}" method="POST" id="AssetReturnRequest" enctype="multipart/form-data">
        ...
        <input type="hidden" value="1" name="header_id" id="header_id" value="{{$Header->requestid}}">
...
<script type="text/javascript">
    ...
    function addRow(){
        ...
        var newrow =
            `<tr>
                <td>
                    <input type="text" placeholder="Search item" class="form-control asset_code text-center" data-id="${tableRow}" id="asset_code${tableRow}"  name="asset_code[]">
        ...
        $(function(){
            countrow++;
            $('#asset_code'+tableRow).autocomplete({
                    ...
                    success: function (data) {
                        ...
                        if(data.items === null){
                            ...
                            if (data.status_no == 1) {
                                ...
                                var searchVal = $('#asset_code'+tableRow).val();
                ...
                select: function (event, ui) {
                    var e = ui.item;
                    if (e.id) {
                        $("#asset_code"+$(this).attr("data-id")).val(e.asset_code);
                        $('#asset_code'+$(this).attr("data-id")).attr('readonly','readonly');
                        $('#digits_code'+$(this).attr("data-id")).val(e.digits_code);
                        $('#item_description'+$(this).attr("data-id")).val(e.item_description);
