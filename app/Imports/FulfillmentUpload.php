...
class FulfillmentUpload implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows->toArray() as $key => $row){
            ...
            $checkQty = DB::table('body_request')->where(['header_request_id'=>$header->id,'digits_code'=>$item_code])->value('unserved_rep_qty');
            ...
            if(!in_array($item_code, $checkRowDbColumnDigitsCode)){
                return CRUDBooster::redirect(CRUDBooster::adminpath('smallwares'),"Digits Code not exist in Item Master: ".($key+2),"danger");
            }
            ...
            if($dr_qty > $checkQty){
                return CRUDBooster::redirect(CRUDBooster::adminpath('smallwares'),"Replenishment Fullfill Qty Exceed! at line: ".($key+2),"danger");
            }
            ...
            ->update(
                        ...
                        'unserved_rep_qty' => DB::raw("unserved_rep_qty - '".(int)$dr_qty."'"), 
