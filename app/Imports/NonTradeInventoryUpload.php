...
class NonTradeInventoryUpload implements ToCollection, SkipsEmptyRows, WithHeadingRow, WithValidation
{
    ...
    public function collection(Collection $rows){
        ...
        $DatabaseCounterCE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$cooking_and_equipment->id)->count();
        $refrigeration_equipment = DB::table('class')->find(2);
		$DatabaseCounterRE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$refrigeration_equipment->id)->count();
        $commercial_ovens = DB::table('class')->find(3);
		$DatabaseCounterCO = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$commercial_ovens->id)->count();
        $refrigeration_and_freezer = DB::table('class')->find(4);
		$DatabaseCounterRAF = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$refrigeration_and_freezer->id)->count();
        $commercial_sinks = DB::table('class')->find(5);
		$DatabaseCounterCS = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$commercial_sinks->id)->count();
        $work_table_stations = DB::table('class')->find(6);
        $DatabaseCounterWTS = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$work_table_stations->id)->count();
        $food_preparation_equipmen = DB::table('class')->find(7);
        $DatabaseCounterFPE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$food_preparation_equipmen->id)->count();
        $faucet_and_plumbing = DB::table('class')->find(8);
        $DatabaseCounterFAP = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$faucet_and_plumbing->id)->count();
        $food_holding_warming_equip = DB::table('class')->find(9);
        $DatabaseCounterFHWE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$food_holding_warming_equip->id)->count();
        $other_restaurant_equipment = DB::table('class')->find(10);
        $DatabaseCounterORE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$other_restaurant_equipment->id)->count();
        $other_vehicle = DB::table('class')->find(11);
        $DatabaseCounterOV = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$other_vehicle->id)->count();
        $other_fixed_asset = DB::table('class')->find(12);
        $DatabaseCounterOFA = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$other_fixed_asset->id)->count();
        $communication_equipment = DB::table('class')->find(13);
        $DatabaseCounterCOMME = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$communication_equipment->id)->count();
        $furnitures_fixtures = DB::table('class')->find(14);
        $DatabaseCounterFF = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$furnitures_fixtures->id)->count();
        $facilities_equipment = DB::table('class')->find(15);
        $DatabaseCounterFE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$facilities_equipment->id)->count();
        $leasehold_equipment = DB::table('class')->find(16);
        $DatabaseCounterLE = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$leasehold_equipment->id)->count();
        $machinery_equipmen = DB::table('class')->find(17);
        $DatabaseCounterME = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$machinery_equipmen->id)->count();
        $vehicle = DB::table('class')->find(18);
        $DatabaseCounterV = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$vehicle->id)->count();
        $computer_software_program = DB::table('class')->find(19);
        $DatabaseCounterCSP = DB::table('assets_non_trade_inventory_body')->where('sub_category_id',$computer_software_program->id)->count();
        ...
        foreach ($rows->toArray() as $row) {
            ...
            $save = AssetsNonTradeInventoryBody::updateOrcreate([
            ...
            [
                ...
                'quantity'                       => DB::raw("IF(quantity IS NULL, '".(int)$row['qty']."', quantity + '".(int)$row['qty']."')"), 
                'serial_no'                      => $serial_no,
    ...
    public function prepareForValidation($data, $index)
    {
        ...
        $checkRowDbDigitsCode = DB::table('assets_non_trade_inventory_body')->select(DB::raw("CONCAT(assets_non_trade_inventory_body.digits_code,'-',LOWER(assets_non_trade_inventory_body.serial_no)) AS codes"))->where('serial_no','!=','N/A')->get()->toArray();
