...
class InventoryImportWoCodeCounter implements ToCollection, SkipsEmptyRows, WithHeadingRow, WithValidation
{
    ...
    {
        //ASSET CODE
        $data['asset_code_exist']['check'] = false;
        $checkRowDbAssetCode = DB::table('assets_inventory_body')->select("asset_code AS asset_code")->get()->toArray();
        $checkRowDbAssetCodeColumn = array_column($checkRowDbAssetCode, 'asset_code');
        $data['asset_code_exist']['code'] = $data['asset_code'];
        if(!empty($data['asset_code'])){
            if(in_array(preg_replace('/\s+/', '', $data['asset_code']), $checkRowDbAssetCodeColumn)){
                $data['asset_code_exist']['check'] = true;
            }else{
                $data['asset_code_exist']['check'] = false;
            }
        }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            '*.asset_code_exist' => function($attribute, $value, $onFailure) {
                if ($value['check'] === true) {
                    $onFailure('Asset Code '.$value['code'].'exist in Asset lists');
                }
            },
        ];
    }
